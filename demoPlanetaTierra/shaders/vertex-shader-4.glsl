...
void main(void) {
    ...
    //Designo atenuación entre oceano y tierra

    float attenuation_coeff = 1.1;
    ...
    if(textureColor.z > textureColor.x && textureColor.z > textureColor.y) {
        position *= (1.0 / attenuation_coeff);
        position += normal * (A * sin(uv.x * K + time * W));
        position += normal * (A * sin(uv.y * K + time * W));
    }
