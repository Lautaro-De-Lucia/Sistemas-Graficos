        ...
        // 3 atributos -> 3 buffers diferentes -> tres arreglos de datos
...
attribute vec2 aUv;         //coordenadas de texture (x,y)  x e y (en este caso) van de 0 a 1
        ...
        /*Recordemos que la GPU tiene cientos de nucleos cada uno de los cuales procesa 
        una cantidad determinada de vértices. Mientras que de aPosition, aNormal y aUv 
        un núcleo de la GPU toma valores específicos (los guarda en su propia RAM y los procesa), 
        todos los nucleos tomarán el mismo valor para estas matrices (se guardan en una
        suerte de RAM compartida por todos los núcleos)
        */ 
...
uniform sampler2D uSampler;         // sampler de textura de la tierra (objeto especial que lee información de una textura)
...
void main(void) {
    ...
    vec4 worldPos = uMMatrix * vec4(position, 1.0); //Recibe mi posición y le aplica las transformaciones
