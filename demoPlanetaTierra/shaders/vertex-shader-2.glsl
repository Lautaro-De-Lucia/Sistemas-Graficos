...
void main(void) {
    ...
    //Llevo a la tierra a su forma medieval

    position.x = uv.x - cos(PI * uv.x / 2.0);
    position.z = uv.y - cos(PI * uv.y / 2.0);
    position.y = 0.0 - 0.5;

    //r es la distancia desde el orígen

    float r = sqrt((position.x) * (position.x) + (position.z) * (position.z));

    //Designo amplitud, velocidad de fase y número de onda

    float A = 0.2;
    float W = 20.0;
    float K = 40.0;

    //Designo un "factor de atenuación"

    float attenuation_coeff = 4.0;

    //Designo a "y" como función de onda

    position.y += A * cos(r * K - (time * W)) / exp(attenuation_coeff * r);
